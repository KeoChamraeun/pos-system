            ...
            <div class="row">
                <div class="col-lg-12">
                    ...
                    <div class="form-group">
                        <button class="btn btn-primary">{{ __('Update Payment') }}<i class="bi bi-check"></i></button>
                    </div>
                </div>
                <div class="col-lg-12">
                    <div class="card">
                                    ...
                                    <div class="from-group">
                                        <div class="form-group">
                                            ...
                                            <select class="form-control" name="payment_method" id="payment_method" required>
                                                <option {{ $saleReturnPayment->payment_method == 'Cash' ? 'selected' : '' }} value="Cash">{{ __('Cash') }}</option>
                                                <option {{ $saleReturnPayment->payment_method == 'Credit Card' ? 'selected' : '' }} value="Credit Card">{{ __('Credit Card') }}</option>
                                                <option {{ $saleReturnPayment->payment_method == 'Bank Transfer' ? 'selected' : '' }} value="Bank Transfer">{{ __('Bank Transfer') }}</option>
                                                <option {{ $saleReturnPayment->payment_method == 'Cheque' ? 'selected' : '' }} value="Cheque">{{ __('Cheque') }}</option>
                                                <option {{ $saleReturnPayment->payment_method == 'Other' ? 'selected' : '' }} value="Other">{{ __('Other') }}</option>
                                            </select>
                                        </div>
                                    </div>
                                </div>
